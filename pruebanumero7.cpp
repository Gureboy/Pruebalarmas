...
#define LED_WIFI D6
#define BEEP_PIN D5  // Pin para el buzzer (beep)
...
// Beep simple: una o más repeticiones
void beep(int repeticiones = 1) {
...
void activar() {
  alarmaActivada = true;
  guardarEstadoEEPROM();
  Blynk.virtualWrite(V2, 1); // Actualiza estado en Blynk
  beep(1); // Un beep al activar
...
void desactivar() {
  ...
  guardarEstadoEEPROM();
  desactivarAlarma();
  Blynk.virtualWrite(V2, 0); // Actualiza estado en Blynk
  beep(2); // Dos beeps al desactivar
